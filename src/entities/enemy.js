...
import { moveWithCollision, lineIntersectsWall } from '../level/walls.js';
...
export function updateEnemy() {
  ...
  if (enemy.investigate) {
    ...
    if (Math.hypot(player.x - enemy.x, player.y - enemy.y) < player.radius + enemy.radius - 2) { return 'death'; }
    return;
  }
  ...
  if (Math.hypot(player.x - enemy.x, player.y - enemy.y) < player.radius + enemy.radius - 2) { return 'death'; }
  if (isInEnemyVision()) { return 'death'; }
