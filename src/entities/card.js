...
import { playCardCollect, playTone } from '../audio/sfx.js';
import { showMessage } from '../ui/screens.js';
import { exit, cards } from '../level/generator.js';
import { holdBar, holdFill } from '../renderer/canvas.js';
    ...
    if (vis && d < player.radius + card.radius + 8) {
      if ((pressedKeys[' '] || pressedKeys['Spacebar']) && !player.breathHold) {
        ...
        if (card.holdProgress >= CONFIG.cardHoldTime) {
          ...
          if (state.cardsFound === level.cards) {
            exit.active = true;
