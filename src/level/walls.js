...
export function lineIntersectsWall(x1, y1, x2, y2) {
  for (const w of walls) {
    if (x1 < w.x - 5 && x2 < w.x - 5 || x1 > w.x + w.w + 5 && x2 > w.x + w.w + 5 ||
        y1 < w.y - 5 && y2 < w.y - 5 || y1 > w.y + w.h + 5 && y2 > w.y + w.h + 5) continue;
    const s = [
...
export function moveWithCollision(obj, dx, dy, sp) {
  ...
  if (cx) obj.x = nx;
  if (cy) obj.y = ny;
  obj.x = Math.max(obj.radius + 2, Math.min(800 - obj.radius - 2, obj.x));
  obj.y = Math.max(obj.radius + 2, Math.min(600 - obj.radius - 2, obj.y));
}
