import { LEVELS, CONFIG } from '../config.js';
import { state } from '../state.js';
import { player } from '../entities/player.js';
import { enemy } from '../entities/enemy.js';
import { walls, circleRectCollision } from './walls.js';
...
export function generateRandomCards(count, minDist = 100) {
  ...
  while (res.length < count && att < 200) {
    ...
    for (const w of walls) {
      if (x > w.x - 25 && x < w.x + w.w + 25 && y > w.y - 25 && y < w.y + w.h + 25) { ok = false; break; }
    }
...
export function generateLevel() {
  ...
  const baseObs = [
    { x: 180, y: 100, w: 30, h: 180 },
    { x: 320, y: 420, w: 140, h: 30 },
    { x: 520, y: 150, w: 30, h: 200 },
    { x: 200, y: 450, w: 100, h: 30 },
    { x: 600, y: 400, w: 30, h: 120 },
    { x: 420, y: 280, w: 60, h: 60 },
    { x: 120, y: 380, w: 80, h: 25 }
  ];
  const extraObs = state.currentLevel >= 1
    ? [{ x: 280, y: 200, w: 25, h: 100 }, { x: 650, y: 100, w: 30, h: 150 }, { x: 150, y: 250, w: 80, h: 25 }]
    : [];
